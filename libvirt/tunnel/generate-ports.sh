...
for i in $(seq 0 $(( $CLUSTER_CAPACITY-1 )) ); do
    yq eval -i '.libvirt-'$ARCH-$CLUSTER_ID-$i'.api.bastion-port='$(($API_PORT + $CLUSTER_ID))'' ${filename}
    yq eval -i '.libvirt-'$ARCH-$CLUSTER_ID-$i'.api.target-port='$(($API_PORT + 1 + 10000 * $i))'' ${filename}
    yq eval -i '.libvirt-'$ARCH-$CLUSTER_ID-$i'.http.bastion-port='$(($HTTP_PORT + $CLUSTER_ID + 8000))'' ${filename}
    yq eval -i '.libvirt-'$ARCH-$CLUSTER_ID-$i'.http.target-port='$(($HTTP_PORT + 10000 * $i + 8000))'' ${filename}
    yq eval -i '.libvirt-'$ARCH-$CLUSTER_ID-$i'.https.bastion-port='$(($HTTPS_PORT + $CLUSTER_ID + 8000))'' ${filename}
    yq eval -i '.libvirt-'$ARCH-$CLUSTER_ID-$i'.https.target-port='$(($HTTPS_PORT + 10000 * $i + 8000))'' ${filename}
